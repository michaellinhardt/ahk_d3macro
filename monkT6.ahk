...
Macro:
	...
	pZ := fastPixelGetColor(699, 1112) ; - = UP
	pZMid := fastPixelGetColor(731, 1173) ; - != PAS UP
	pE := fastPixelGetColor(773, 1112) ; 1448470 = UP ALLIE MYSTIQUE
	pR := fastPixelGetColor(847, 1112) ; 1448470 = UP EPIPHANIE
	
	pEsprit := fastPixelGetColor(1518, 1072) ; 8224199 = ESPRIT >200
	; pAutoHit := fastPixelGetColor(695, 1107) ; 855309 = AutoHit requis
	
	;msgbox pCD = %pCD% pCG = %pCG% pa = %pA% pZ = %pZ% pZMid = %pZMid% pE = %pE% pR = %pR%
	;msgbox  pAutoHit = %pAutoHit%  
	
	if (pE = "1448470" && pEsprit != "8224199") {
		Send e
		Sleep 50
	}
	if (pR = "1448470" && pEsprit != "8224199") {
		Send r
		Sleep 50
	}
	if (Mantra = 6) {
